...
def load_exceptions():
    ...
    """
    candidates = [ROOT / "tools" / "coverage_exceptions.yaml", ROOT / "coverage_exceptions.yaml"]
    for p in candidates:
        if p.exists():
            try:
                data = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
            ...
            for dom, lst in (data.get("domains") or {}).items():
                if isinstance(lst, list):
                    domains[dom] = {item.strip() for item in lst if isinstance(item, str)}
...
def main():
    ...
    for m in sorted(metas):
        ...
        links_count = sum(1 for item in links if isinstance(item, str) and item.strip() and "<" not in item)
    ...
    all_ok = sum(1 for r in rows if r["name_ok"] and r["description_ok"] and r["links_ok"] and r["covered"])
    ...
    print(f"Covered in compliance maps: {coverage_ok}/{total} ({pct(coverage_ok)})")
    print(f"All checks passed: {all_ok}/{total} ({pct(all_ok)})")
    print()

    # Detailed lists
    orphans = [r for r in rows if not r["covered"]]
    placeholder_rows = [r for r in rows if (not r["name_ok"]) or (not r["description_ok"])]
    stale = [r for r in rows if (not r["covered"]) or (not r["name_ok"]) or (not r["description_ok"])]
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(f"# Policy coverage audit\nGenerated: {report['generated_at']}\n\n")
        ...
        write_section("Policies with placeholder name/description", placeholder_rows)
    ...
    with open(trimmed_md, "w", encoding="utf-8") as tf:
        tf.write(f"# Policy coverage audit — Orphans & Stale\nGenerated: {report['generated_at']}\n\n")
        ...
        write_section_tf("Stale policies (coverage empty OR placeholder)", stale)
