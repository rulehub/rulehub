...
test_allow_when_compliant if {
	allow with input as {"controls": {"betting.self_exclusion_uk_gamstop": true}, "player": {"self_excluded": false}, "session": {"blocked": true}}
}

test_denies_when_session_blocked_false if {
	count(deny) > 0 with input as {"controls": {"betting.self_exclusion_uk_gamstop": true}, "player": {"self_excluded": true}, "session": {"blocked": false}}
}

test_denies_when_control_disabled if {
	count(deny) > 0 with input as {"controls": {"betting.self_exclusion_uk_gamstop": false}, "player": {"self_excluded": false}, "session": {"blocked": true}}
}
