...
test_allow_when_compliant if {
	allow with input as {"controls": {"betting.license_check_spillemyndigheden_dk": true}, "operator": {"licensed": true}}
}

test_denies_when_operator_licensed_false if {
	count(deny) > 0 with input as {"controls": {"betting.license_check_spillemyndigheden_dk": true}, "operator": {"licensed": false}}
}

test_denies_when_control_disabled if {
	count(deny) > 0 with input as {"controls": {"betting.license_check_spillemyndigheden_dk": false}, "operator": {"licensed": true}}
}
...
test_denies_when_controls_betting_license_check_spillemyndigheden_dk_failing if {
	some _ in deny with input as {"controls": {"betting.license_check_spillemyndigheden_dk": false}, "operator": {"licensed": true}}
}
