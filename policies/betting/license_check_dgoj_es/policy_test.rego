...
test_allow_when_compliant if {
	allow with input as {"controls": {"betting.license_check_dgoj_es": true}, "operator": {"licensed": true}}
}

test_denies_when_operator_licensed_false if {
	count(deny) > 0 with input as {"controls": {"betting.license_check_dgoj_es": true}, "operator": {"licensed": false}}
}
