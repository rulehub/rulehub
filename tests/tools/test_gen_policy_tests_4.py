...
def test_dry_run_creates_plan(tmp_path, monkeypatch):
    ...
    pol_dir = tmp_path / "policies" / "demo" / "four_denies"
    pol_dir.mkdir(parents=True)
    pol = pol_dir / "policy.rego"
    pol.write_text('\n'.join([
        'package rulehub.demo.four_denies',
        '',
    ...
    # run dry-run
    script = Path.cwd() / 'tools' / 'gen_policy_tests_4.py'
    proc = subprocess.run([
        sys.executable,
        str(script),
        '--policies-root',
...
def test_policy_flag_mismatch_exits_nonzero(tmp_path):
    ...
    assert 'has 1 deny rules (expected 4)' in proc.stdout
